...
class Fp8Config(QuantizationConfig):
    ...
    def from_config(cls, config: Dict[str, Any]) -> Fp8Config:
        ...
        if ignored_layers:
            ...
            # ATOM patch: HF-style configs (e.g. Step-3.5-Flash-FP8) list the
            # *merged* fused linear name (e.g. share_expert.gate_up_proj),
            # but is_layer_skipped expands fused layers into their unfused
            # shard names (gate_proj, up_proj, q_proj, k_proj, v_proj) and
            # checks each shard. Pre-expand the merged names so all shards
            # match and the layer is correctly skipped from FP8 quant.
            _FUSED_EXPAND = {
                "gate_up_proj": ("gate_proj", "up_proj"),
                "qkv_proj": ("q_proj", "k_proj", "v_proj"),
            }
            extra = []
            for layer in list(normalized):
                for fused, shards in _FUSED_EXPAND.items():
                    suffix = "." + fused
                    if layer == fused or layer.endswith(suffix):
                        for shard in shards:
                            extra.append(layer[: -len(fused)] + shard if layer.endswith(suffix) else shard)
