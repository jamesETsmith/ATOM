...
class Step3p5ForCausalLM(nn.Module):
    ...
    def load_weights(self, weights: Iterable[Tuple[str, torch.Tensor]], is_nextn=False):
        ...
        # ATOM patch: Step-3.5-Flash-FP8 is a mixed-precision checkpoint
        # (FP8 routed experts only; attention qkv/o, dense MLP, share_expert,
        # lm_head all bf16 via modules_to_not_convert in config.json). The
        # FP8 quant method registers `weight_scale_inv` placeholders even on
        # bf16 layers, leaving them unbound after weight load. Suppress the
        # assert for those scale-only placeholders when their underlying
        # weight tensor was loaded successfully.
        scale_suffixes = (".weight_scale_inv", ".weight_scale", ".input_scale")
        filtered = set()
        for p in print_params:
            matched = False
            for sfx in scale_suffixes:
                if p.endswith(sfx):
                    base = p[: -len(sfx)] + ".weight"
                    if base in loaded_params:
                        matched = True
                        break
            if not matched:
                filtered.add(p)
        assert len(filtered) == 0, f"Some parameters are not loaded: {filtered}"
